...
export const Consents: FunctionComponent<ConsentComponentProps> = (props: ConsentComponentProps): JSX.Element => {
    ...
    const getConsentedApps = async (): Promise<void> => {
        try {
            const apps: ConsentInterface[] = await fetchConsentedApps(ConsentState.ACTIVE);
    ...
    /**
     * The following method handles the toggle state of each pii category item.
     *
     * @param {number} piiCategoryId - pii category id.
     * @param {number} purposeId - mapping purpose id
     * @param {string} receiptId - mapping consent receipt
     */
